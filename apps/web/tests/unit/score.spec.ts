...
test("[ARM-SCORE-001] Score is round(okCount / itemCount * 100)", () => {
...
test("[ARM-SCORE-002] Empty submission (zero items) yields score 100", () => {
  expect(computeScore({ okCount: 0, itemCount: 0 })).toBe(100);
});

test("[ARM-SCORE-003] All-ok submission yields score 100", () => {
  expect(computeScore({ okCount: 6, itemCount: 6 })).toBe(100);
  expect(computeScore({ okCount: 12, itemCount: 12 })).toBe(100);
});

test("[ARM-SCORE-004] All-fail submission yields score 0", () => {
